...
//Настройки wi-fi
const char* ssid = "GeoScan_guest";
const char* password = "plaz2398";
...
void setup() {
  ...
  if (ENABLE_SERIAL_DEBUG) Serial.println("WiFi connected");
...
void loop() {
  if (SerialBT.available()) {
    //if (ENABLE_SERIAL_DEBUG) Serial.println("something arrived!");
    symb = SerialBT.read();
    Serial.print(symb);
    if (symb == 'z') {
      SerialBT.println(WiFi.localIP());
      if (ENABLE_SERIAL_DEBUG) Serial.println(WiFi.localIP());
    }
  //delay(1);
  }
  //delay(1);
